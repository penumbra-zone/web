...
export const senderAlreadyApproved = async (
  sender?: chrome.runtime.MessageSender,
): Promise<boolean> => {
  if (!sender?.origin?.startsWith('https://') || !sender.tab?.id || sender.frameId)
...
export const approveSender = async (sender?: chrome.runtime.MessageSender): Promise<UserChoice> => {
