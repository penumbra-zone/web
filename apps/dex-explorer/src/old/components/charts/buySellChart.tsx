...
export default dynamic(
  async () => {
    ...
    const BuySellChart = ({
      ...
      const cleaned_buy_side_positions = buySidePositions
        ...
        .map(position => {
          ...
          const willingToBuy = Number.parseFloat(
            (Number(direction === 1 ? reserves2.toFixed(6) : reserves1.toFixed(6)) / price).toFixed(
              6,
            ),
          );
      ...
      let midPointPrice = 0;
      if (cleaned_buy_side_positions.length > 0 && cleaned_sell_side_positions.length > 0) {
        midPointPrice =
        ...
        midPointPrice = cleaned_sell_side_positions[cleaned_sell_side_positions.length - 1].price;
      }
      ...
      const DataRow: React.FC<{
        ...
        <Box position='relative' width='100%'>
          <Box
            ...
            height='100%'
            backgroundColor={type === 'buy' ? 'rgba(0, 255, 0, 0.3)' : 'rgba(255, 0, 0, 0.3)'}
            right='0'
          />
          <HStack
            width='100%'
          ...
          >
            <Text color={type === 'buy' ? 'green.500' : 'red.500'}>{price.toFixed(6)}</Text>
            <Spacer />
      ...
      const maxBuyAmount = Math.max(...cleaned_buy_side_positions.map(pos => pos.willingToBuy));
      const maxSellAmount = Math.max(...cleaned_sell_side_positions.map(pos => pos.willingToSell));
      const maxAmount = Math.max(maxBuyAmount, maxSellAmount);

      return (
        <VStack flex={1} height='600px'>
          <HStack
            width='100%'
            fontFamily='monospace'
            fontSize='10px'
          ...
          >
            ...
            <Spacer />
            <Text paddingRight='115px' textAlign={'right'}>{`Amount (${asset1Token.symbol})`}</Text>
            <Text>{`LP ID`}</Text>
          </HStack>
          ...
          >
            <VStack
              ...
              spacing={1}
              overflowY='auto'
              justifyContent={sellSidePositions.length < 15 ? 'flex-end' : 'flex-start'} // Conditionally adjust based on the number of sell rows
            >
