...
export const getAllAssets = async (): Promise<Metadata[]> => {
  const responses = await Array.fromAsync(praxClient.service(ViewService).assets({}));
...
export const getAssetMetadataById = async (assetId: AssetId): Promise<Metadata | undefined> => {
  ...
  const { denomMetadata } = await praxClient.service(ViewService).assetMetadataById(req);
