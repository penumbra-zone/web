...
export interface IbcOutSlice {
  ...
  filteredBalances?: PlainMessage<BalancesResponse>[];
    ...
    destination: {
      ...
      setChain: ({ chainName, chainId }: { chainName?: string; chainId?: string }) => {
        ...
        const filteredBalances = filterBalancesPerChain(
          Array.from(get().ibcOut.availableBalances ?? []).map(b => new BalancesResponse(b)),
          metadata,
        ).map(toPlainMessage);

        set(state => {
          state.ibcOut.destination.chain = metadata;
          state.ibcOut.filteredBalances = filteredBalances;
        });
      },
    },
