...
describe('lohi helpers', () => {
  it('fromBaseUnitAmount works', () => {
    const result = fromBaseUnitAmount(create(AmountSchema, { lo: 1000n, hi: 5n }), 6);
  ...
  it('joinLoHiAmount works', () => {
    ...
    expect(joinLoHiAmount(create(AmountSchema, { lo, hi }))).toBe(
...
describe('addAmounts', () => {
  it('should return an Amount with lo and hi equal to 0 when both inputs are 0', () => {
    const a = create(AmountSchema, { lo: 0n, hi: 0n });
    const b = create(AmountSchema, { lo: 0n, hi: 0n });
  ...
  it('should correctly add two Amounts where there is no carry from lo to hi', () => {
    const a = create(AmountSchema, { lo: 1n, hi: 1n });
    const b = create(AmountSchema, { lo: 2n, hi: 2n });
  ...
  it('should correctly add two Amounts where there is carry from lo to hi', () => {
    const a = create(AmountSchema, { lo: 18446744073709551615n, hi: 1n }); // max value for a 64-bit integer
    const b = create(AmountSchema, { lo: 1n, hi: 2n });
  ...
  it('should correctly add two Amounts where hi is at the maximum value', () => {
    const a = create(AmountSchema, { lo: 0n, hi: 18446744073709551615n }); // max value for a 64-bit integer
    const b = create(AmountSchema, { lo: 0n, hi: 18446744073709551615n }); // max value for a 64-bit integer
...
describe('subtractAmounts', () => {
  ...
  it('should correctly subtract two Amounts', () => {
    const a = create(AmountSchema, { lo: 2n, hi: 2n });
    const b = create(AmountSchema, { lo: 1n, hi: 1n });
...
describe('divideAmounts', () => {
  ...
  it('should return 0n if dividend is zero', () => {
    const a = create(AmountSchema, { lo: 0n, hi: 0n });
    const b = create(AmountSchema, { lo: 18446744073709551615n, hi: 1n });
  ...
  it('should return a number without fractions when dividing without remainder', () => {
    const a = create(AmountSchema, { lo: 6n, hi: 0n });
    const b = create(AmountSchema, { lo: 2n, hi: 0n });
    const result = divideAmounts(a, b);
  ...
  it('should return a number with specified precision', () => {
    const a = create(AmountSchema, { lo: 10n, hi: 0n });
    const b = create(AmountSchema, { lo: 3n, hi: 0n });
    const result = divideAmounts(a, b);
...
describe('isZero', () => {
  it('works with zero amount', () => {
    const amount = create(AmountSchema, { lo: 0n, hi: 0n });
    expect(isZero(amount)).toBeTruthy();
  });

  it('works with negatives', () => {
    const amount = create(AmountSchema, { lo: -13n, hi: 1n });
    expect(isZero(amount)).toBeFalsy();
  });

  it('detects hi number presence', () => {
    const amount = create(AmountSchema, { lo: 0n, hi: 1n });
    expect(isZero(amount)).toBeFalsy();
  });

  it('detects lo number presence', () => {
    const amount = create(AmountSchema, { lo: 20n, hi: 0n });
    expect(isZero(amount)).toBeFalsy();
  });
});
...
describe('toDecimalExchangeRate()', () => {
  it('correctly expresses the exchange rate as a decimal', () => {
    const amount = create(AmountSchema, { hi: 0n, lo: 325_000_000n });
...
describe('multiplyAmountByNumber()', () => {
  it('correctly multiplies an amount by a number', () => {
    const amount = create(AmountSchema, { hi: 0n, lo: 100n });
...
describe('fromString', () => {
  it('converts a string to an amount', () => {
    ...
    const expected = create(AmountSchema, { hi: 0n, lo: 123456n });

    expect(equals(AmountSchema, result, expected)).toBe(true);
  });
