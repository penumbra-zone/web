...
import { IDBUpdate, IDBUpdater } from './updater';
import IDB_VERSION from './version';

type PenumbraUpdate<N extends StoreNames<PenumbraSchema> = StoreNames<PenumbraSchema>> = IDBUpdate<
  PenumbraSchema,
  N
>;

export class PenumbraDb {
  private constructor(
    private readonly db: IDBPDatabase<PenumbraSchema>,
    private readonly u: IDBUpdater<PenumbraSchema>,
  ...
  static async initialize({
    ...
    const db = await openDB<PenumbraSchema>(idbName, idbVersion, {
      upgrade(db: IDBPDatabase<PenumbraSchema>) {
    ...
    const instance = new this(db, new IDBUpdater(db), chainId);
  ...
  subscribe<DBTypes extends PenumbraSchema, StoreName extends StoreNames<DBTypes>>(
    table: StoreName,
  ): AsyncGenerator<PenumbraUpdate, void> {
  ...
  // SCT updates must be atomic in order to prevent invalid tree state
  public async saveScanResult(updates: ScanBlockResult): Promise<void> {
    const ups = new Array<PenumbraUpdate>();

    this.addSctUpdates(ups, updates.sctUpdates);
    this.addNewNotes(ups, updates.newNotes);
    await this.addNewSwaps(ups, updates.newSwaps, updates.height);
    ups.push({ table: 'FULL_SYNC_HEIGHT', value: updates.height, key: 'height' });

    await this.u.updateAll(ups);
  }
  ...
  private addSctUpdates(
    ups: IDBUpdate<PenumbraSchema>[],
    sctUpdates: ScanBlockResult['sctUpdates'],
  ): void {
    ...
    for (const c of sctUpdates.store_commitments) {
      ups.push({ table: 'TREE_COMMITMENTS', value: c });
    }

    for (const h of sctUpdates.store_hashes) {
      ups.push({ table: 'TREE_HASHES', value: h });
    }

    // TODO: What about updates.delete_ranges?
  }

  private addNewNotes(ups: PenumbraUpdate[], notes: SpendableNoteRecord[]): void {
    for (const n of notes) {
      ups.push({ table: 'SPENDABLE_NOTES', value: n.toJson() as Jsonified<SpendableNoteRecord> });
    }
  }
  ...
  private async addNewSwaps(
    ...
    for (const n of swaps) {
      ...
      ups.push({ table: 'SWAPS', value: n.toJson() as Jsonified<SwapRecord> });
    }
  }
