...
import type { IDBUpdate } from './updater';
import {
  AuctionId,
...
describe('IndexedDb', () => {
  ...
  describe('initializing', () => {
    it('sets up as expected in initialize()', async () => {
      const db = await PenumbraDb.initialize(generateInitialProps());
      await expect(db.getFullSyncHeight()).resolves.not.toThrow();
    });

    it('different chain ids result in different databases', async () => {
      const testnetDb = await PenumbraDb.initialize(generateInitialProps());
      const mainnetDb = await PenumbraDb.initialize({
    ...
    it('same version uses same db', async () => {
      ...
      const dbA = await PenumbraDb.initialize(props);
      await dbA.saveAssetsMetadata(metadataA);

      const dbB = await PenumbraDb.initialize(props);
    ...
    it('increasing version should re-create object stores', async () => {
      ...
      const dbA = await PenumbraDb.initialize(version1Props);
      await dbA.saveAssetsMetadata(metadataA);
      dbA.close();
      ...
      const dbB = await PenumbraDb.initialize(version2Props);
  ...
  describe('Updater', () => {
    it('emits events on update', async () => {
      ...
      const updateA = resA.value as IDBUpdate<PenumbraSchema, 'SPENDABLE_NOTES'>;
      ...
      const updateB = resB.value as IDBUpdate<PenumbraSchema, 'SPENDABLE_NOTES'>;
  ...
  describe('assets', () => {
    it('should be pre-loaded with hardcoded assets', async () => {
      ...
      const db = await PenumbraDb.initialize(propsWithAssets);
  ...
  describe('epochs', () => {
    ...
    beforeEach(async () => {
      db = await PenumbraDb.initialize({ ...generateInitialProps() });
    });
