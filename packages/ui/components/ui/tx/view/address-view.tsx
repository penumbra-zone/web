import { Address, AddressView, AddressView_Opaque } from '@buf/penumbra-zone_penumbra.bufbuild_es/penumbra/core/keys/v1alpha1/keys_pb';
...
const AddressViewComponent: React.FC<{
    ...
    copyable?: boolean,
    short_form?: boolean
}> = ({ view, address, copyable, short_form }) => {
    ...
    return (
        <div className='address-view flex'>
            {(account_index !== undefined) ? (
                ...
                <div className='text-sm italic text-foreground font-mono'>{display}</div>
            )
            }
            {copyable ? (
                <CopyToClipboard
