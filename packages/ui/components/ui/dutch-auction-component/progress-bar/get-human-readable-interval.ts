...
export const getHumanReadableInterval = (seconds: number): string => {
  ...
  if (hours >= 1 || parts.length > 0) parts.push(`${hoursRounded}h`);
  ...
  if (minutes >= 1 || parts.length > 0) parts.push(`${minutesRounded}m`);
  if (minutes === minutesRounded) return parts.join(' ');
  remainder = remainder % MINUTE_IN_SECONDS;

  if (remainder >= 1 || parts.length > 0) parts.push(`${remainder}s`);
  return parts.join(' ');
};
