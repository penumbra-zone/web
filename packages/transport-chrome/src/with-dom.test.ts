...
describe('message transport', () => {
  ...
  /** it seems like vite has trouble posting readable streams through a standard message port? */
  it.todo('should send and receive streaming requests', async () => {
    ...
    const streamListener = vi.fn((sub: chrome.runtime.Port) => {
      if (sub.name === streamChannel) {
        ...
        void stream
          ...
          .catch((e: unknown) => console.log('wtf test', e))
          .finally(() => {
            console.log('stream write done');
            sub.disconnect();
          });
      }
    });
...
describe.todo('transport aborts', () => {
  let domPort: MessagePort;
  let transportOptions: ChannelTransportOptions;
  let transport: Transport;
    ...
    await vi.waitFor(() => {
      expect(extOnMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: expect.objectContaining(input),
        }),
        expect.any(Object),
      );
    });
