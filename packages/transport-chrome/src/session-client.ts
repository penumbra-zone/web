...
export class CRSessionClient {
  ...
  private constructor(private readonly clientPort: MessagePort) {
    ...
    AbortSignal.any([instanceAc.signal, CRSessionClient.signal]).addEventListener('abort', () => {
      if (instanceAc.signal.aborted) {
        if (globalThis.__DEV__) {
          console.warn('session-client signal', this.sessionName, instanceAc.signal.reason);
        }
        // if abort was local and not global, report reason
        this.reportError({
          requestId: undefined,
  ...
  /** Clear the session for possible reconnect. */
  private reset = () => {
  ...
  private connect = () => {
    if (globalThis.__DEV__) {
      console.debug('session-client connecting', this.sessionName, this.signal.reason);
    }
    ...
    try {
      ...
      if (globalThis.__DEV__) {
        console.debug('session-client connected', port);
      }
      return port;
    } catch (e) {
      if (globalThis.__DEV__) {
