...
type withSenderVal = typeof CRSessionManager & {
  init: (
    prefix: string,
  ...
  ) => ReturnType<typeof CRSessionManager.init>;
};
...
describe('CRSessionManager', () => {
  ...
  describe('origin validation', () => {
    ...
    it.fails.each(allSenders)(
      `fails: should accept or reject sender %# according to external sender validation callback permitting ${badSenders.map(s => allSenders.indexOf(s)).join(', ')}`,
      async someSender => {
        ...
        const sessions = (CRSessionManager as withSenderVal).init(
          testName,
          mockHandler,
          checkPortSender,
        );
