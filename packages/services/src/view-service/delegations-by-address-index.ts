...
import {
  ...
  getValidatorInfoFromGetValidatorInfoResponse,
...
export const delegationsByAddressIndex: Impl['delegationsByAddressIndex'] = async function* (
  ...
  if (!stakeClient) {
    throw new Error('Stake context not found');
  }
  ...
  for await (const validatorInfoResponse of stakeClient.validatorInfo({ showInactive })) {
    const validatorInfo = getValidatorInfoFromGetValidatorInfoResponse(validatorInfoResponse);
