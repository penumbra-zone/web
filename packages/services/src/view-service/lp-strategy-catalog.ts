...
import { LpStrategyCatalogResponse, LpStrategyCatalogResponse_StrategyEntry } from '@penumbra-zone/protobuf/penumbra/view/v1/view_pb';
...
export const lpStrategyCatalog: Impl['lpStrategyCatalog'] = async function* (req, ctx) {
    ...
    let strategies: LpStrategyCatalogResponse_StrategyEntry[] | undefined = [];
    for await (const positionBundle of indexedDb.getPositionsByStrategyStream(req.subaccount!, req.positionMetadata!, undefined, req.tradingPair)) {
        let entry = new LpStrategyCatalogResponse_StrategyEntry({
