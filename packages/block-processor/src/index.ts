...
import {
  MerkleRoot,
  StateCommitment,
} from '@buf/penumbra-zone_penumbra.bufbuild_es/penumbra/crypto/tct/v1/tct_pb';
import {
  SpendableNoteRecord,
  SwapRecord,
...
import { PenumbraDb } from '@penumbra-zone/idb';
import {
  AppService,
  AuctionService,
  CnidariumService,
...
import type { ViewServer } from '@penumbra-zone/wasm/view-server';
import { backOff } from 'exponential-backoff';
import {
  processActionDutchAuctionEnd,
  processActionDutchAuctionSchedule,
  processActionDutchAuctionWithdraw,
} from './dutch-auction';
import { updatePricesFromSwaps } from './prices';
...
export class BlockProcessor {
  ...
  private fullnode: {
    tendermint: PromiseClient<typeof TendermintProxyService>;
    app: PromiseClient<typeof AppService>;
    compactBlock: PromiseClient<typeof CompactBlockService>;
    shieldedPool: PromiseClient<typeof ShieldedPoolService>;
    auction: PromiseClient<typeof AuctionService>;
    cnidarium: PromiseClient<typeof CnidariumService>;
  ...
  constructor(
    private readonly indexedDb: PenumbraDb,
    private readonly viewServer: ViewServer,
    private readonly numeraires: Metadata[],
    private readonly stakingTokenMetadata: Metadata,
    private readonly fullnodeTransport: Transport,
  ) {
    this.fullnode = {
      tendermint: createPromiseClient(TendermintProxyService, this.fullnodeTransport),
      app: createPromiseClient(AppService, this.fullnodeTransport),
      compactBlock: createPromiseClient(CompactBlockService, this.fullnodeTransport),
      shieldedPool: createPromiseClient(ShieldedPoolService, this.fullnodeTransport),
      auction: createPromiseClient(AuctionService, this.fullnodeTransport),
      cnidarium: createPromiseClient(CnidariumService, this.fullnodeTransport),
      stake: createPromiseClient(StakeService, this.fullnodeTransport),
    };
  }
  ...
  private async syncAndStore() {
    ...
    let latestKnownBlockHeight = await backOff(
      async () => {
        const latest = (await this.fullnode.tendermint.getStatus({})).syncInfo?.latestBlockHeight;
    ...
    for await (const { compactBlock } of this.fullnode.compactBlock.compactBlockRange(
      {
      ...
      if (compactBlock.appParametersUpdated) {
        await this.indexedDb.saveAppParams(
          (await this.fullnode.app.appParameters({})).appParameters,
        );
      }
      ...
      if (spentNullifiers.size || recordsByCommitment.size) {
        ...
        const { blockHeight: responseHeight, transactions: blockTx } =
          await this.fullnode.app.transactionsByHeight({ blockHeight: compactBlock.height });
        if (responseHeight !== compactBlock.height)
          throw new Error(
            `Block height mismatch: requested transactions for ${compactBlock.height}, received ${responseHeight}`,
          );
      ...
      if (blockInPriceRelevanceThreshold && compactBlock.swapOutputs.length) {
        await updatePricesFromSwaps(
          (...p) => this.indexedDb.updatePrice(...p),
          this.numeraires,
  ...
  private async saveAndReturnMetadata(assetId: AssetId): Promise<Metadata | undefined> {
    ...
    const metadataFromNode = await this.fullnode.shieldedPool.assetMetadataById({ assetId }).then(
  ...
  private async identifyAuctionNfts(action: Action['action']) {
    if (action.case === 'actionDutchAuctionSchedule' && action.value.description) {
      ...
      const auctionStateById = async (id: AuctionId) => {
        const { auction } = await this.fullnode.auction.auctionStateById({ id });
        return auction?.unpack(typeRegistry);
      };
      await processActionDutchAuctionEnd(action.value, auctionStateById, this.indexedDb);
  ...
  private async assertRootValid(blockHeight: bigint): Promise<void> {
    const fetchRemoteRoot = async (height: bigint) => {
      const merkleRootBin = (
        await this.fullnode.cnidarium.keyValue({
          key: `sct/tree/anchor_by_height/${height}`,
        })
      ).value?.value;
      return merkleRootBin && MerkleRoot.fromBinary(merkleRootBin);
    };
    const remoteRoot = await fetchRemoteRoot(blockHeight);
    const inMemoryRoot = this.viewServer.getSctRoot();

    if (inMemoryRoot.equals(remoteRoot)) {
      console.debug(
      ...
      console.error(
        ...
        `Remote hash: ${remoteRoot?.inner && uint8ArrayToHex(remoteRoot.inner)}`,
      );
    }
  }
  ...
  private async updateValidatorInfos(nextEpochStartHeight: bigint): Promise<void> {
    for await (const { validatorInfo } of this.fullnode.stake.validatorInfo({
      showInactive: true,
    })) {
      ...
      void this.updatePriceForValidatorDelegationToken(validatorInfo, nextEpochStartHeight);
    }
  }
  ...
  private async updatePriceForValidatorDelegationToken(
    ...
    const delegationTokenAssetId = new AssetId({
      altBaseDenom: `udelegation_${bech32mIdentityKey(
    ...
    if (metadata) {
      ...
      const exchangeRate = getExchangeRateFromValidatorInfo(validatorInfo);
