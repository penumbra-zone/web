...
describe('assetPatterns', () => {
  ...
  describe('unbondingTokenPattern', () => {
    it('matches when a string begins with `unbonding_`', () => {
      expect(assetPatterns.unbondingTokenPattern.test('unbonding_abc123')).toBe(true);
    });

    it('does not match when a string contains, but does not begin with, `unbonding_`', () => {
      expect(
        assetPatterns.unbondingTokenPattern.test('ibc-transfer/channel-1234/unbonding_abc123'),
      ).toBe(false);
    });
  });
