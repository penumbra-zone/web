import {
  seal_share,
  split_input_rep3_bls12_377,
  split_witness_rep3_bls12_377,
  split_witness_shamir_bls12_377,
  verify_proof_result_signature,
} from '../wasm/index.js';
...
async function scheduleJob(
  ...
  if (!keyMaterial || !shares || keyMaterial.length < 3 || shares.length < 3) {
    throw new Error('Missing or insufficient key material or shares for 3-party job.');
  }
 ...
 */
export function verifyProofResultSignature(
  jobId: string,
...
export async function scheduleFullJobRep3(
  ...
  return await scheduleJob(apiInstance, blueprintId, JobType.Rep3Full, shares, keyMaterial);
}
...
export async function scheduleProveJobRep3(
  ...
  const shares = [sharedInput.shares0, sharedInput.shares1, sharedInput.shares2];
  return await scheduleJob(apiInstance, blueprintId, JobType.Rep3Prove, shares, keyMaterial);
}
...
export async function scheduleProveJobShamir(
  ...
  const shares = [sharedInput.shares0, sharedInput.shares1, sharedInput.shares2];
  return await scheduleJob(apiInstance, blueprintId, JobType.ShamirProve, shares, keyMaterial);
}
