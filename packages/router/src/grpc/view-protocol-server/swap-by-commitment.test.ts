...
import {
    ...
    SpendableNoteRecord, SwapByCommitmentRequest, SwapByCommitmentResponse, SwapRecord,
...
describe('SwapByCommitment request handler', () => {
    ...
    test('should successfully get swap by commitment when idb has them', async () => {
        mockIndexedDb.getSpendableNoteByCommitment?.mockResolvedValue(testSwap);
    ...
    test('should throw error if commitment is missing in request', async () => {
        await expect(swapByCommitment(new NoteByCommitmentRequest(), mockCtx)).rejects.toThrow(
    ...
    test('should get swap if swap is not found in idb, but awaitDetection is true, and has been detected', async () => {
        ...
        expect(noteByCommitmentResponse.spendableNote?.equals(testSwap)).toBeTruthy();
    });
...
const swapWithAnotherCommitment = SpendableNoteRecord.fromJson({
    noteCommitment: {
