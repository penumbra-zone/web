...
export default async function simulationHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.status(500).json({ error: `Error simualtion trade grpc data: ${error}` });
  }
}
